...
struct默认public，class默认private
        结构体一般用于存储数据，类用于封装数据和行为
        结构体成员默认public，类成员默认private
        结构体适合简单数据聚合，类适合复杂对象建模
        结构体通常没有成员函数，类通常有成员函数
        结构体更接近C语言风格，类是C++的核心特性
        结构体实例化时可以省略struct关键字，类实例化时必须使用类名
        结构体更适合POD类型，类适合需要封装和继承的场景
        结构体和类在语法上非常相似，但语义不同
    二者不同在于结构体是深拷贝，而类是浅拷贝
    所以在类中需要手动实现复制构造函数和赋值运算符重载
